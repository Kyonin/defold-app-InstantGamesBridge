...
let LibraryInstantGamesBridge = {
    ...
    $InstantGamesBridgeLib: {
        ...
        init_callbacks: function (igb, callback_ids) {
            console.log("init_callbacks", igb);
            ...
            //igb.advertisement.on(igb.EVENT_NAME.INTERSTITIAL_STATE_CHANGED, state => console.log('Interstitial state:', state));
            //igb.advertisement.on(igb.EVENT_NAME.INTERSTITIAL_STATE_CHANGED, state => igb.send(callback_ids.interstitial_state_changed, state));
        ...
        send: function (callback_id, data) {
            ...
            if (InstantGamesBridgeLib._callback && callback_id > 0) {
                ...
                console.log("callbacks", callback_id);
        ...
        call_api: function (method, parameters, callback_id, native_api) {
            ...
            console.log("result_object ", parent_object);
            console.log("parameters ", string_parameters);
                ...
                case "function":
                    try {
                        ...
                        if (callback_id === 0) {
                            ...
                            called_function(...array_parameters);
                            InstantGamesBridgeLib.send(callback_id, true);/*.then(
                                function (success) {
                                    if (save_as_var) {
    ...
    InstantGamesBridge_Init: function (parameters, callback_id) {
        ...
        InstantGamesBridgeLib.init = function (igb, initResult) {
            ...
            console.log("callback_ids", callback_ids);
            if (initResult == "success") //КОСТЫЛЬ, новый ответ от нового API заменяет ответ на нужный
                initResult = true;
            if (initResult === true) {
