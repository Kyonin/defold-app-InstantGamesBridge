...
let LibraryInstantGamesBridge = {
    ...
    $InstantGamesBridgeLib: {
        ...
        call_api: function (method, parameters, callback_id, native_api) {
            //instantGamesBridge['advertisement']['_platformBridge']['playerName'] 
            ...
            if (method_name + "" === "getProperty") {
                return JSON.stringify(instantGamesBridge[array_parameters[0]]["_platformBridge"][array_parameters[1]]);
            }
    ...
    InstantGamesBridge_Init: function (parameters, callback_id) {
        ...
        InstantGamesBridgeLib.init = function (igb, initResult) {
            ...
            if(initResult == "success") //КОСТЫЛЬ!!! Очень важный... Пожалуйста не трогайте и не меняйте!!!
                initResult = true;
            if (initResult === true) {
